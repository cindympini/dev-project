...
class TestDevCollab(unittest.TestCase):
    ...
    def test_8_collaborate(self):
        ...
        # Create project via POST request instead of directly with the database
        response = self.client.post('/create_project', data={
            'title': 'Collab Project',
            'description': 'Need help',
            'stage': 'idea',
            'support': 'Looking for collaborators'
        }, follow_redirects=True)
        ...
        with app.app_context():
            new_project = Project.query.filter_by(title='Collab Project').first()
        ...
        with app.app_context():
            ...
            self.assertEqual(collab.project_id, new_project_id)
            self.assertEqual(collab.user_id, self.user2_id)
